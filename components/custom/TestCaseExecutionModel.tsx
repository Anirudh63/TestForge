...
type RunResult = {
    testCaseId: number;
    status: "idle" | "generating" | "running" | "passed" | "failed" | "skipped";
    logs: string[];
    error?: string;
    sessionId?: string;
    sessionUrl?: string;
...
export default function TestExecutionModal({ isOpen, onClose, testCases, repository }: Props) {
    ...
    // Queue of test case IDs to execute (allows re-run failed subset)
    const [executionQueue, setExecutionQueue] = useState<TestCase[]>([]);

    // Progress calculations
    const progress = useMemo(() => {
        const total = executionQueue.length || testCases.length;
        const completed = Object.values(results).filter(
            r => r.status === "passed" || r.status === "failed" || r.status === "skipped"
        ).length;
        ...
        return { total, completed, passed, failed, skipped, percentage };
    }, [results, executionQueue, testCases]);
    ...
    useEffect(() => {
        if (isOpen && testCases.length > 0) {
            ...
            // Default to "generate" mode so that latest prompt selector enhancements are applied
    ...
    useEffect(() => {
        if (!isExecuting || currentIdx < 0 || currentIdx >= executionQueue.length) {
            if (currentIdx >= executionQueue.length) {
                setIsExecuting(false);
            }
            return;
        }
        ...
        const runTest = async () => {
            const currentTestCase = executionQueue[currentIdx];
            const tcId = currentTestCase.id;

            // Check if skip was requested for this test
            if (skipRequested) {
                setSkipRequested(false);
                setResults((prev) => ({
                    ...prev,
                    [tcId]: {
                        ...prev[tcId],
                        status: "skipped",
                        logs: [...(prev[tcId]?.logs || []), "[SYSTEM] ⏭ Test skipped by user"],
                    },
                }));
        ...
        runTest();
    }, [isExecuting, currentIdx, executionQueue, baseUrl, executionMode, skipRequested]);

    const startExecution = (queue?: TestCase[]) => {
        const queueToUse = queue || testCases;
        setExecutionQueue(queueToUse);

        // Reset all statuses for the queue
        const resetResults: Record<number, RunResult> = { ...results };
        queueToUse.forEach((tc) => {
            resetResults[tc.id] = {
                testCaseId: tc.id,
                status: "idle",
                logs: ["Queued..."],
        ...
        setIsExecuting(true);
        setCurrentIdx(0);
        setSelectedDetailId(queueToUse[0].id);
        setSkipRequested(false);
    };
    ...
    const handleRerunFailed = () => {
        const failedTestCases = testCases.filter(tc => results[tc.id]?.status === "failed");
        if (failedTestCases.length > 0) {
            startExecution(failedTestCases);
        }
    };
    ...
    const hasFailedTests = Object.values(results).some(r => r.status === "failed");
    const executionComplete = !isExecuting && progress.completed > 0 && progress.completed >= executionQueue.length;

    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="max-w-5xl h-[90vh] flex flex-col p-6 gap-4 bg-white rounded-2xl shadow-lg border border-[#e2e8f0] overflow-hidden select-none">
                <DialogHeader className="border-b border-[#e2e8f0] pb-4 flex flex-row items-center justify-between shrink-0">
                    <div>
                        <DialogTitle className="text-xl font-semibold text-[#1a1a2e] flex items-center gap-2">
                            <PlayCircle className="text-[#6366f1] h-5 w-5" />
                            Test Runner
                        </DialogTitle>
                        <DialogDescription className="text-[#94a3b8] text-sm">
                            Run automation scripts on the deployed target URL using cloud browser infrastructure.
                        </DialogDescription>
                    </div>
                </DialogHeader>
                            ...
                            <div className="flex gap-3 text-[10px] font-medium">
                                {progress.passed > 0 && (
                                    <span className="text-[#10b981] flex items-center gap-0.5">
                                        <CheckCircle2 className="h-3 w-3" /> {progress.passed} passed
                                    </span>
                                )}
                                {progress.failed > 0 && (
                                ...
                                {progress.skipped > 0 && (
                                    <span className="text-gray-500 flex items-center gap-0.5">
                                        <SkipForward className="h-3 w-3" /> {progress.skipped} skipped
                                    </span>
                                )}
                            </div>
                        </div>
                ...
                <div className="flex flex-col bg-[#f8fafc] p-4 rounded-xl border border-[#e2e8f0] gap-3 shrink-0">
                    <div className="flex flex-col sm:flex-row gap-4 items-end">
                        <div className="flex-1 space-y-1.5">
                            <label className="text-xs font-medium text-[#94a3b8] uppercase tracking-wider flex items-center gap-1.5">
                                <Globe className="h-3.5 w-3.5 text-[#6366f1]" /> Target Website URL
                            </label>
                            <Input
                                ...
                                className="bg-white border-[#e2e8f0] font-mono text-sm h-10"
                            />
                        </div>
                        <div className="flex gap-2.5">
                            <Button
                                ...
                                className={`h-10 px-4 font-medium text-xs gap-1.5 transition-colors border-[#e2e8f0] ${showOptions ? "bg-[#eef2ff] text-[#6366f1] border-[#c7d2fe]" : ""}`}
                            >
                            ...
                            {isExecuting && (
                                <Button
                                    ...
                                    variant="outline"
                                    className="h-10 px-3 text-xs gap-1 border-amber-300 text-amber-700 hover:bg-amber-50"
                                >
                            ...
                            {!isExecuting ? (
                                <Button
                                    ...
                                    className="h-10 bg-[#6366f1] hover:bg-[#5558e6] text-white font-medium px-6 gap-2"
                                >
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-5 overflow-hidden">
                    ...
                    <div className="md:col-span-1 border border-[#e2e8f0] rounded-xl overflow-y-auto bg-[#f8fafc] p-3 flex flex-col gap-2">
                        <h3 className="text-[11px] font-semibold text-[#94a3b8] uppercase tracking-wider px-2 mb-1">
                            Execution Queue
                        </h3>
                        {testCases.map((tc, index) => {
                            const res = results[tc.id];
                            const isActive = selectedDetailId === tc.id;
                            const queueIdx = executionQueue.findIndex(q => q.id === tc.id);
                            const isRunning = queueIdx === currentIdx && isExecuting;
                            ...
                            return (
                                ...
                                >
                                    <div className="flex justify-between items-start gap-2 mb-1">
                                        <h4 className="font-medium text-sm text-[#1a1a2e] line-clamp-1">
                                            {tc.title}
                                        </h4>
                                        <ChevronRight className={`h-4 w-4 text-[#94a3b8] transition-transform ${isActive ? "rotate-90 text-[#6366f1]" : ""}`} />
                                    </div>
                                    <p className="text-xs text-[#94a3b8] line-clamp-1 mb-2.5">
                                        {tc.description}
                                    </p>
                    ...
                    <div className="md:col-span-2 border border-[#e2e8f0] rounded-xl flex flex-col bg-white overflow-hidden">
                        {currentSelectedTestCase ? (
                            <div className="flex-1 flex flex-col overflow-hidden">
                                {/* Header Info */}
                                <div className="p-4 border-b border-[#e2e8f0] bg-[#f8fafc] flex justify-between items-start gap-4 shrink-0">
                                    <div>
                                    ...
                                    {currentSelectedResult?.sessionUrl && (
                                        <Button
                                            ...
                                            variant="outline"
                                            size="sm"
                                            className={`font-medium text-xs gap-1 shadow-xs shrink-0 ${
                ...
                {/* Footer Controls */}
                <div className="border-t border-[#e2e8f0] pt-4 flex justify-between items-center shrink-0">
                    <div className="flex gap-2">
                        {executionComplete && hasFailedTests && (
                            <Button
                                onClick={handleRerunFailed}
                                variant="outline"
                                className="h-10 font-medium px-4 text-xs gap-1.5 border-red-200 text-red-700 hover:bg-red-50"
                            >
                                <RotateCcw className="h-3.5 w-3.5" /> Re-run Failed ({progress.failed})
                            </Button>
                        )}
                    </div>
...
function StatusBadge({
    ...
    switch (status) {
        ...
        case "skipped":
            return (
                <Badge className="bg-gray-200 text-gray-600 hover:bg-gray-200 border-none flex gap-1 items-center">
